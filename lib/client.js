...
Client.prototype.setHttpHeader = function(headerName, headerValue) {
  if(!this.httpHeaders){
    this.httpHeaders = {};
  }
...
Client.prototype._invoke = function(method, arguments, location, callback) {
    ...
    // Set extra HTTP headers if present
    if (self.httpHeaders) {
      for (var headerName in self.httpHeaders) {
        headers[headerName] = self.httpHeaders[headerName];
      }
    }
    ...
    // console.log(ns, name, method);
    this.wsdl.xmlnsInEnvelope=' xmlns:tns="' + method.soapAction + '"';
    // console.log('wsdl.xmlnsInEnvelope', this.wsdl.xmlnsInEnvelope);
